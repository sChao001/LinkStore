...
- (void)awakeFromNib {
    ...
    leftBtn.sd_layout.leftSpaceToView(view, 10).centerYEqualToView(view).widthIs(24).heightIs(24);
//    [leftBtn setImage:[UIImage imageNamed:@"y_library"] forState:UIControlStateNormal];
    [leftBtn setBackgroundImage:[UIImage imageNamed:@"y_library"] forState:UIControlStateNormal];
...
//    [leftBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    UIButton *rightBtn = [[UIButton alloc] init];
    [view addSubview:rightBtn];
    rightBtn.sd_layout.rightSpaceToView(view, 10).centerYEqualToView(view).widthIs(40).heightIs(24);//40  30
    [rightBtn setTitle:@"完成" forState:UIControlStateNormal];
    [rightBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [rightBtn addTarget:self action:@selector(_dismissKeyBoard) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_inputTextView.rac_textSignal subscribeNext:^(id x) {
        ...
        // textView高度自适应
        CGRect bounds = self.inputTextView.bounds;
        
        // 计算 text view 的高度
        CGSize maxSize = CGSizeMake(bounds.size.width, CGFLOAT_MAX);
        CGSize newSize = [self.inputTextView sizeThatFits:maxSize];
        bounds.size = newSize;
        self.inputTextView.bounds = bounds;
