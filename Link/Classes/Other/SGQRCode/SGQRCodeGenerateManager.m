...
+ (UIImage *)generateWithLogoQRCodeData:(NSString *)data logoImageName:(NSString *)logoImageName logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    ...
    // 再把小图片画上去
    UIImageView * header = [[UIImageView alloc]initWithFrame:CGRectMake(-28, 1, 27, 27)];
//    [header sd_setImageWithURL:[NSURL URLWithString:logoImageName] placeholderImage:[UIImage imageNamed:@"header"]];
    header.image = [UIImage imageNamed:logoImageName];
    UIImage *icon_image = header.image;
