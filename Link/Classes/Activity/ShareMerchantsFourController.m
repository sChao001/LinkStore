//
//  ShareMerchantsFourController.m
//  Link
//
...
@interface ShareMerchantsFourController () <WKNavigationDelegate, WKScriptMessageHandler, AppDelegateOfDelegate>
...
@implementation ShareMerchantsFourController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.webView.configuration.userContentController addScriptMessageHandler:self name:@"payComMoney"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self.webView.configuration.userContentController removeScriptMessageHandlerForName:@"payComMoney"];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"action"]) {
        NSLog(@"%@", message.body);
        ShareMerchtsFiveController *vc = [[ShareMerchtsFiveController alloc] init];
        vc.loadUrlStr = message.body;
        vc.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if ([message.name isEqualToString:@"payComMoney"]) {
        NSLog(@"%@", message.body);
        MembersCardPayController *vc = [[MembersCardPayController alloc] init];
        vc.shopIdStr = [NSString stringWithFormat:@"%@", message.body[@"shopId"]];
//        vc.totalFeeStr = message.body[@"totalFee"];
        vc.realPrice = message.body[@"totalFee"];
        vc.comId = message.body[@"comId"];
