...
@interface GroupMemberAddController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation GroupMemberAddController
    ...
    if (self.indexPathRow == indexPath.row) {
        ...
        if (cell.sureImg.selected == YES) {
            ...
            if (_GroupId.count >= 1) {
                ...
                UIImage *selectImage = [image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
                UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithImage:selectImage style:UIBarButtonItemStylePlain target:self action:@selector(requestNetDataFromGroupToAddPeople)];
...
// 拉人进群
- (void)requestNetDataFromGroupToAddPeople {
    NSString * parameString = @"";
    for (int i = 0; i < _GroupId.count; i++) {
        NSString * userid = @"";
        //        if (i==0) {
    ...
    NSLog(@"nameString:%@", nicknameStr);
    NSDictionary *paramet = @{@"sign" : BD_MD5Sign.md5String, @"userId" : [UserInfo sharedInstance].getUserid, @"userIds" : parameString, @"groupId" : _targetId, @"groupName" : _targetName, @"nickName" : [UserInfo sharedInstance].getNickName, @"userNickNames" : nicknameStr};
    [SCNetwork postWithURLString:BDUrl_s(@"groupUser/add") parameters:paramet success:^(NSDictionary *dic) {
        if ([dic[@"code"] integerValue] > 0) {
            NSLog(@"群拉人：%@", dic);
